...
class EmptyMapping(dict):
    def __getitem__(self, item):
        return ""


def message_server(message):
    print("[SP] {}".format(Template(message).safe_substitute(EmptyMapping())))


def message_client(index, message):
    message2 = Template("${white}[${purple}SP${white}] {}").safe_substitute(colors).\
        format(Template(message).safe_substitute(colors))
    SayText2(message=message2).send(index)


def message_console(index, message):
    TextMsg(message="[SP] {}".format(Template(message).safe_substitute(EmptyMapping())), destination=2).send(index)
