...
def sp_ignite(source, command):
    if command.get_arg_count() == 1:
        source.message("Usage: ${purple}sp_ignite ${white}<name|#userid|@filter> [time: 0]")
    ...
    if len(targets) == 0:
        ...
        source.message("Set " + str(len(targets)) + " players on fire.")
    return CommandReturn.BLOCK
...
def sp_ignite(source, command):
    if command.get_arg_count() == 1:
        source.message("Usage: ${purple}sp_freeze ${white}<name|#userid|@filter> [time: 0]")
        return CommandReturn.BLOCK
    targets = target_filter(command[1], source.index)
    time = int(command[2]) if command.get_arg_count() > 2 else 0
    if len(targets) == 0:
        ...
        source.message("Froze " + str(len(targets)) + " players.")
    return CommandReturn.BLOCK
...
def sp_ignite(source, command):
    if command.get_arg_count() == 1:
        source.message("Usage: ${purple}sp_unfreeze ${white}<name|#userid|@filter>")
        return CommandReturn.BLOCK
    targets = target_filter(command[1], source.index)
    if len(targets) == 0:
        source.message("No Targets found.")
